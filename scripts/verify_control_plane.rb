...
fail!("gate_failure_router_base64_validation_missing") unless router.include?("base64.b64decode") && router.include?("validate=True")
release_stager_hydration = File.read(WORKFLOWS.join("omega-recovery-evidence-release-stager.yml"), encoding: "UTF-8")
fail!("release_stager_bound_artifact_hydration_missing") unless release_stager_hydration.include?("OMEGA_RELEASE_STAGE_BOUND_ARTIFACT_MISSING") && release_stager_hydration.include?("shutil.copy2")
